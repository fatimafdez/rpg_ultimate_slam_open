...
# txt file names
ODOMETRY_FILE="odometry.txt";
VICON_FILE="vicon.txt";

# Vicon topic name
VICON_TOPIC="/vicon_client/dvxplorer02/pose";
...
if [ "$SESSIONEXISTS" = "" ]; then
    ...
    tmux send-keys -t $SESSION:0.2 "cd $BAGS_PATH && rosbag play -d 2 $BAG_NAME use_sim_time:=true --clock -s 1 --wait" C-m

    tmux send-keys -t $SESSION:0.3 "cd $WS_PATH && source $SOURCE_ROUTE1 && rosrun ze_vio_ceres odometry_subscriber_c $ODOMETRY_FILE $VICON_FILE $VICON_TOPIC --wait" C-m
